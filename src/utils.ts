...
const addDays = function (originalDate: Date, days: number) {
  var date = originalDate
  date.setDate(date.getDate() + days);
  return date;
};

const addHours = function (originalHour: Date, hours: number) {
...
const timeLabelGenerator = (firstDay: number[]) => {
...
const getDate = (unixObject: number) => {
  const a = new Date(unixObject * 1000);
  return a.getDate();
};
const getDay = (unixObject: number) => {
  const a = new Date(unixObject * 1000);
